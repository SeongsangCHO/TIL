...
- 카카오톡 (daelee, taelee, jehong, mihykim)

---

###### 5. 학습 목표 

- 서버에서 데이터베이스로부터 받은 데이터를 리액트화면에 로드한다.
...
- 리액트의 라이프사이클이벤트와 노드와의 json데이터 통신을 수행하는 법을 알게되었습니다.
- 프로젝트는 리액트, node를 이용해 페이지를 구축할 것이므로 미리 알아야 어떻게 코드를 작성할지 가이드라인을 잡을 수 있을 것 같습니다.
- 프록시를 이용해 서버-클라이언트간의 포트를 중개한다는 것을 알게되었습니다.
